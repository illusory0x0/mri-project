...
- **`/ask-me`**: the **no-lookup** interview. **Facts are yours to supply, not the agent's**: it never reads the codebase, the environment, or dispatches a sub-agent. It asks you for any fact a question needs, and marks a decision blocked if you don't have it. Reach for it in a **quick prompt**, when the answers are already in the conversation and a lookup would only re-read what you both already know.
