...
For a **quick prompt**, where the full flow is overkill, see [FAST-LANE.md](FAST-LANE.md): it ranks the skills by how much they read the codebase versus work from the conversation, and by whether they pay a sub-agent tax.
...
- **`/ask-me`**: the **no-lookup** variant of the interview. Same rounds and frontier as `/grilling`, but **facts are yours to supply, not the agent's**: it never reads the codebase, the environment, or dispatches a sub-agent. It asks you for any fact a question needs, and marks a decision blocked if you don't have it. Reach for it in a **quick prompt**, when the answers are already in the conversation and a lookup would only re-read what you both already know.
