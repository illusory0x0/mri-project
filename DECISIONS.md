...
**Decision.** For now, extract the inline client into a standalone `.ts`
file and bundle it with esbuild, adding the `DOM` lib so `tsc` type-checks
it. Introduce no framework. If an interactive editor UI is built, use
