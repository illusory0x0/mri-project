...
- [x] `--repeats N` runs each task × arm N times; each execution is a distinct
      run carrying its repeat index.
- [x] Artifacts are named `arm-task.json` when `N = 1` and `arm-task-r<N>.json`
      otherwise.
- [x] With repeats present, report cells show agreement as `n/N` and a
      per-(set, arm) stability table summarizes agreement.
- [x] With `N = 1` the filenames and the report display are unchanged.
- [x] A test asserts N repeats produce N artifacts with distinct repeat indices,
