...
export const TOOL_SPECS: Record<string, ToolSpec> = {
  lisp_editor: {
    ...
    description:
      ...
      "Shapes: lambda -> (lambda (_param) _body); if -> (if _cond _then _else); define -> (define _name _body); let -> (let ((_name _value)) _body); apply:<n> -> (_func _arg1 ... _argn), e.g. apply:2 -> (_func _arg1 _arg2); hole -> _ (a visible placeholder).\n" +
