...
export const TOOL_SPECS: Record<string, ToolSpec> = {
  lisp_editor: {
    ...
    description:
      ...
      "Shapes: lambda -> (lambda (_param) _body); if -> (if _cond _then _else); define -> (define _name _body); define-fn -> (define (_name _param) _body), a function header; let -> (let ((_name _value)) _body); let* -> (let* ((_name _value)) _body); cond -> (cond (_test1 _body1) (_test2 _body2)); list -> (_item1 _item2), a general two-element list of holes; apply:<n> -> (_func _arg1 ... _argn), a list of n+1 elements (the head plus n argument holes), e.g. apply:2 -> (_func _arg1 _arg2); hole -> _ (a visible placeholder).\n" +
